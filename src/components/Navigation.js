...
const Navigation = ({ changeCategory, category }) => {
    ...
    return (
        <animated.div style={appearFromTop}>
            <header>
                <h1 onClick={() => changeCategory('business')}>The Journal App</h1>
                {
            ...
            <animated.nav style={appearanceMenu}>
                <ul>
                    ...
                    <li onClick={() => { setMenuOpen(!menuOpen); changeCategory('Sport') }} style={{ backgroundColor: category === 'Sport' ? '#fff' : '#000', color: category === 'Sport' ? '#000' : '#fff' }}>SPORT</li>
